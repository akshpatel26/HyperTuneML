...
# To Disable Warnings
# Removed the deprecated set_option line
import warnings
...
def choice_classifier(data, data_name, X, Y, fig, ax):
    ...
    # We cannot give data directly we have to specify the values for x and y
        ...
        sns.scatterplot(
            x=X[:, 0], y=X[:, 1], hue=Y, palette=sns.color_palette(colors), alpha=0.4, ax=ax
        )
        ax.legend(shadow=True)
        ax.set_title("Scatter Classification Plot of Dataset With Target Classes")
...
def data_model_description(algorithm, algorithm_type, data_name, data, X, Y):
    ...
    # Plotting - create a figure and axis
    fig, ax = plt.subplots()
    ...
    if algorithm_type == "Regressor":
        fig = choice_regressor(X, x_test, predict, data, data_name, Y, fig, ax)
    else:
        # Calling Function
        fig = choice_classifier(data, data_name, X, Y, fig, ax)
